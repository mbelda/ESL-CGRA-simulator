...
def gesummv_reference(A, B, x, y, alpha, beta, N):
    ...
    Returns both y_out and the intermediate vector tmp.
    """
...
def generate_and_write(N, seed=None):
    ...
    # Reference computation (now returns y_expected and tmp_expected)
    y_expected, tmp_expected = gesummv_reference(A, B, x, y, alpha, beta, N)
    ...
    def write_uninitialized_array(f, name, size, interleaved=False):
        """Escribe un vector reservado en C (inicializado a 0)."""
        attr = ' __attribute__((section(".xheep_data_interleaved")))' if interleaved else ''
        f.write(f"int {name}[{size}]{attr} = {{0}};\n\n")
    ...
    with open(h_filename, "w") as f:
        ...
        # Buffer intermedio 'tmp' reservado en memoria interleaved
        write_uninitialized_array(f, "tmp", N, interleaved=True)
