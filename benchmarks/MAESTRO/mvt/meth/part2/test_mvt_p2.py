...
# repo/benchmarks/MAESTRO/mvt/part2 -> repo/
...
def configMemory(A, x2, y2, N):
    ...
    config_vals[0] = [first_addr_A, first_addr_x2 + (4 * 4), N, N]
    config_vals[1] = [N, N, first_addr_A + (9 * 4), first_addr_x2 + (13 * 4), loopJit, loopIit, first_addr_y2]
    config_vals[2] = [first_addr_x2 + (2 * 4), first_addr_A + (6 * 4), N, N]
    config_vals[3] = [N, N, first_addr_x2 + (11 * 4), first_addr_A + (15 * 4), first_addr_y2]
...
runKernel(load_addrs, max_it=2000000000, pr=["ROUT", "R3", "INST"], printVal=1)
