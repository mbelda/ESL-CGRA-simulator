...
# repo/benchmarks/MAESTRO/gemm/ -> repo/
...
parser = argparse.ArgumentParser(description="Script para ejecutar GEMM con dimensiones dinámicas.")
...
# ------------------------------------------------------------------
log_filename = f"gemm_{NI}x{NJ}_output.log"
...
#           FUNCTIONS
# ------------------------------------
def configMemory(A, B, C, NI, NK, NJ, alpha, beta):
    ...
    first_addr_C = first_addr_B + size_B  
    
    first_addr_res = first_addr_C
    ...
    addr_C_0_2 = first_addr_C + 8
    addr_C_1_3 = first_addr_C + (1 * NJ * 4) + 12
    addr_C_2_0 = first_addr_C + (2 * NJ * 4)
    addr_C_3_1 = first_addr_C + (3 * NJ * 4) + 4
    ...
    # Config vals estructurado en 4 columnas
    config_vals = [[] for i in range(CGRA_N_COLS)]

    config_vals[0] = [addr_A_0_0, loopIit, addr_C_2_0, addr_B_0_0, alpha, beta, NK]
    config_vals[1] = [addr_B_0_1, addr_A_1_0, addr_C_3_1, NK, beta, alpha, NJ]
    config_vals[2] = [addr_C_0_2, addr_B_0_2, addr_A_2_0, loopKit, NJ, alpha, NK, beta]
    config_vals[3] = [loopJit, addr_C_1_3, addr_B_0_3, addr_A_3_0, beta, NJ, alpha]
    ...
    kernel_add_memory_region(kernel_name, first_addr_C, C, version=version)
...
alpha = data["alpha"] if "alpha" in data else 1
beta = data["beta"] if "beta" in data else 1
