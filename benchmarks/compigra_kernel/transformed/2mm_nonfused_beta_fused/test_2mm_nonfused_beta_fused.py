...
def configMemory(tmp_data, A_data, B_data, C_data, D_data, NI, NJ, NK, NL):
    ...
    config_cols = [
        [],
        [first_addr_tmp, first_addr_B],
        [first_addr_tmp, first_addr_D],
        [first_addr_A, first_addr_C, first_addr_tmp],
    ]
...
def main():
    ...
    # Redirigir la salida estándar al archivo log
    with open(log_file_path, "w", encoding="utf-8") as log_f:
        with redirect_stdout(log_f):
            ...
            load_addrs = configMemory(
                tmp_data, A_data, B_data, C_data, D_data, NI, NJ, NK, NL
            )
            runKernel(load_addrs, max_it=200000000, printVal=1)
            ...
            expected_res = twomm_cpu(
                A_cpy, B_cpy, C_cpy, D_cpy, NI, NJ, NK, NL, alpha, beta
            )
    ...
    # Información resumida en la terminal
