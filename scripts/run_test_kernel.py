...
def configMemory(
    ...
    # -------------------------------------------------------------------------
    # CONFIGURACIÓN ESPECÍFICA PARA KERNEL (Registros de Control)
    # -------------------------------------------------------------------------
    kernel_add_memory_region(
        ".", 65024, [first_addr_inputX], version=version_tag
    )  # A_base
    kernel_add_memory_region(
        ".", 65028, [first_addr_inputY], version=version_tag
    )  # B_base
    kernel_add_memory_region(
        ".", 65032, [first_addr_output], version=version_tag
    )  # C_base
    kernel_add_memory_region(".", 65036, [NI], version=version_tag)  # NI
    kernel_add_memory_region(".", 65040, [NK], version=version_tag)  # NK
    kernel_add_memory_region(".", 65044, [NJ], version=version_tag)  # NJ
...
def main():
    parser = argparse.ArgumentParser(
        description="Ejecutor de benchmarks específico para KERNEL"
    )
    parser.add_argument(
    ...
    with open(log_file_path, "w", encoding="utf-8") as log_f:
        with redirect_stdout(log_f):
            print(
                f"[-] [MODE: KERNEL] Ejecutando: {kernel_name} ({version_tag})"
            )
